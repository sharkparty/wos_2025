...
describe("User Collection Schema Validation", () => {
  ...
  test("should fail validation if a required field is missing from an item", () => {
    ...
    if (validateUserCollection.errors) {
      ...
      expect(validateUserCollection.errors[0].params.missingProperty).toBe(
        "roleId",
      );
    } else {
      throw new Error(
        "Expected validation to fail and produce errors, but validateUserCollection.errors was null/undefined.",
      );
    }
  });
  ...
  test("should fail validation if an item contains an extra property", () => {
    ...
    if (validateUserCollection.errors) {
      expect(Array.isArray(validateUserCollection.errors)).toBe(true);
      expect(validateUserCollection.errors.length).toBeGreaterThan(0);
      expect(validateUserCollection.errors[0].keyword).toBe(
        "additionalProperties",
      );
      expect(validateUserCollection.errors[0].params.additionalProperty).toBe(
        "extraField",
      );
    } else {
      throw new Error(
